...
export default function App() {
  ...
  const handleLogin = (e) => {
    ...
    if (loginRole === 'student') {
      const stu = students.find(s => s.prn === loginId.trim());
      if (!stu) { setLoginError('Student PRN not found'); return; }
      setCurrentUser(stu);
      ...
      <div className="login-page">
        <div className="login-box">
          ...
          <form onSubmit={handleLogin}>
            <div className="form-group">
              <label className="form-label">
                {loginRole === 'student' ? 'PRN' : loginRole === 'faculty' ? 'Employee ID' : 'Username'}
              </label>
              <div className="input-with-icon">
                <i className="ti ti-id input-icon" />
                <input type="text" value={loginId} onChange={e => { setLoginId(e.target.value); setLoginError(''); }}
                  placeholder={loginRole === 'student' ? 'Enter your PRN' : loginRole === 'faculty' ? 'Enter Employee ID' : 'Enter username'} />
              </div>
            </div>
            <div className="form-group">
          ...
          <div className="login-demo-hint">
            ...
            Student: PRN <strong>2024001</strong> · Faculty: <strong>FAC001</strong> · Admin: <strong>admin@dept</strong><br />
            (any password works)
          </div>
        </div>
